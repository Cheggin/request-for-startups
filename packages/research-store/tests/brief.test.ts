...
import { addPage } from "../src/wiki.js";
import { appendResult } from "../src/ledger.js";
...
describe("generateBrief", () => {
  it("generates a brief for an empty category", () => {
    const brief = generateBrief(TEST_ROOT, {
      category: "coding",
    ...
    // Should have a "survey" idea for empty category
  ...
  it("includes ledger experiment summary", () => {
    appendResult(TEST_ROOT, {
      category: "coding",
      experiment_description: "add ESLint strict rules",
      metric: "lint_errors",
      result: "0",
      status: "keep",
      confidence: 0.9,
    ...
    appendResult(TEST_ROOT, {
      category: "coding",
      experiment_description: "switch to tabs",
      metric: "readability",
      result: "worse",
      status: "discard",
      confidence: 0.3,
    ...
    expect(brief.prior_findings_summary).toContain("2 total experiments");
    expect(brief.prior_findings_summary).toContain("1 kept");
    expect(brief.prior_findings_summary).toContain("1 discarded");
    expect(brief.prior_findings_summary).toContain("add ESLint strict rules");
    expect(brief.prior_findings_summary).toContain("switch to tabs");
  });

  it("generates avoid-failure idea when failures exist", () => {
    appendResult(TEST_ROOT, {
      category: "growth",
      experiment_description: "aggressive popup CTA",
      metric: "bounce_rate",
      result: "0.85",
      status: "discard",
      confidence: 0.7,
    ...
    const avoidIdea = brief.ideas.find((i) =>
      i.title.includes("avoid prior failures"),
    );
  ...
  it("includes cross-category findings", () => {
    addPage(
      ...
      "Improve user experience with usability testing and interviews",
      ["ux", "research"],
      { confidence: 0.8, source: "web" },
    );
    ...
    // Should find the design page as cross-category finding
  ...
  it("sorts ideas by confidence (high first)", () => {
    // Create enough state to generate multiple ideas
    addPage(TEST_ROOT, "coding", "Low Conf Page", "Needs validation", ["test"], {
      confidence: 0.2,
      source: "session",
    });
    ...
    appendResult(TEST_ROOT, {
      category: "coding",
      experiment_description: "failed attempt",
      metric: "score",
      result: "0",
      status: "discard",
      confidence: 0.3,
    ...
    if (brief.ideas.length >= 2) {
      ...
      for (let i = 1; i < brief.ideas.length; i++) {
        expect(
          confidenceOrder[brief.ideas[i].confidence],
        ).toBeGreaterThanOrEqual(
          confidenceOrder[brief.ideas[i - 1].confidence],
        );
      }
    }
  });
  ...
  it("accepts pre-provided history", () => {
    const history = [
      {
        ...
        experiment_description: "external experiment",
        metric: "score",
        result: "100",
    ...
    const brief = generateBrief(TEST_ROOT, {
      category: "coding",
      goal: "Test with history",
      history,
    });

    expect(brief.prior_findings_summary).toContain("1 total experiment");
