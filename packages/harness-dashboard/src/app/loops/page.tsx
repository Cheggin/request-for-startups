  ...
  return (
    <div className="px-6 py-5 max-w-6xl">
      ...
      <div className="flex items-baseline gap-6 mb-5">
        <div>
          <span className="text-3xl font-semibold tabular text-text-primary">{loading ? "-" : running.length}</span>
          <span className="text-base text-text-tertiary ml-1.5">running</span>
        </div>
        <div className="text-base text-text-tertiary">{loops.length} total</div>
      </div>

      {loading ? (
        <p className="text-base text-text-tertiary py-4">Loading loop definitions...</p>
      ) : error ? (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border text-left">
                <th className="px-4 py-2 text-xs font-medium text-text-tertiary uppercase tracking-wider">Loop</th>
                <th className="px-4 py-2 text-xs font-medium text-text-tertiary uppercase tracking-wider">Description</th>
                <th className="px-4 py-2 text-xs font-medium text-text-tertiary uppercase tracking-wider">Interval</th>
                <th className="px-4 py-2 text-xs font-medium text-text-tertiary uppercase tracking-wider">Skill</th>
                  ...
                  <tr key={loop.name} className="border-b border-border-subtle last:border-0 hover:bg-surface-hover transition-colors">
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-base font-semibold text-text-primary">{loop.name}</span>
                        <span className="text-2xs font-medium uppercase tracking-wider px-1.5 py-0.5 rounded bg-bg text-text-tertiary">{loop.loopType}</span>
                      </div>
                    </td>
                    <td className="px-4 py-2.5 max-w-sm">
                      <p className="text-sm text-text-secondary truncate">{loop.description}</p>
                    </td>
                    <td className="px-4 py-2.5">
                      <span className="text-sm text-text-secondary font-mono">{loop.interval}</span>
                    </td>
                    <td className="px-4 py-2.5">
                      <span className="text-xs text-text-tertiary font-mono truncate max-w-48 block">{loop.skill}</span>
                    </td>
                    <td className="px-4 py-2.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <span className={`text-sm font-medium capitalize ${loop.status === "running" ? "text-positive" : "text-text-tertiary"}`}>
                          {loop.status}
                        </span>
                        <button
