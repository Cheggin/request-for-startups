...
export default function SettingsPage() {
  ...
  return (
    <div className="px-6 py-5 max-w-6xl space-y-8">
      <h1 className="text-[18px] font-semibold text-text-primary leading-tight">Settings</h1>

      <section>
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-[13px] font-semibold text-text-primary">Service Credentials</h2>
          <span className="text-[11px] text-text-tertiary tabular">{configuredServices}/{snapshot.services.length} configured</span>
        </div>
            ...
            return (
              <div key={service.name} className="px-4 py-3 hover:bg-surface-hover transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`ml-auto text-[11px] font-medium ${state.className}`}>{state.label}</span>
                </div>
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {service.envs.map((envName) => (
                    <code key={envName} className={`text-[11px] px-1.5 py-0.5 rounded font-mono ${service.configuredKeys.includes(envName) ? "bg-positive/10 text-positive" : "bg-bg text-text-tertiary"}`}>
                      {envName}
                    </code>
                  ))}
                </div>
      ...
      <section>
        <h2 className="text-[13px] font-semibold text-text-primary mb-2">Harness Config</h2>
        <div className="border border-border-subtle rounded-md divide-y divide-border-subtle">
          {snapshot.harnessFiles.map((file) => (
            <div key={file.path} className="px-4 py-3 flex items-center gap-3 hover:bg-surface-hover transition-colors">
              <div className="min-w-0">
                ...
                <div className="text-[11px] text-text-tertiary font-mono truncate">{file.path}</div>
              </div>
              <span className="ml-auto text-[11px] text-text-tertiary">{file.summary}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      {snapshot.startupConfigs.length > 0 && (
        <section>
          <h2 className="text-[13px] font-semibold text-text-primary mb-2">Startup Configs</h2>
            ...
            {snapshot.startupConfigs.map((config) => (
              <div key={config.path} className="border border-border-subtle rounded-md overflow-hidden">
                <div className="px-4 py-3 border-b border-border-subtle bg-surface">
                  <div className="text-[13px] font-medium text-text-primary">{config.startupName}</div>
                  <div className="text-[11px] text-text-tertiary font-mono">{config.path}</div>
                </div>
