import { getDeployments, getStartups } from "@/lib/data";
...
export default function DeployPage() {
  ...
  const startupEndpoints = getStartups().filter((startup) => Boolean(startup.deployUrl));

  return (
    <div className="px-6 py-5 max-w-6xl">
      <h1 className="text-[18px] font-semibold text-text-primary leading-tight mb-6">
        Deployment Health
      </h1>
      ...
      <section className="mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          {deployments.length > 0 && (
            <span className="text-[12px] text-text-tertiary tabular">
              {deployments.length} result{deployments.length === 1 ? "" : "s"}
            </span>
          )}
        </div>
        ...
        {deployments.length === 0 ? (
          <div className="border border-border-subtle rounded-md px-4 py-5">
            ...
            <p className="text-[12px] text-text-tertiary mt-1">
              The dashboard reads deployments from the real <code className="font-mono text-[12px] text-text-secondary bg-bg px-1 py-0.5 rounded">vercel ls --json</code> output. If this stays empty, authenticate the CLI with <code className="font-mono text-[12px] text-text-secondary bg-bg px-1 py-0.5 rounded">vercel login</code>.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full">
              ...
              <tbody>
                {deployments.map((deployment, index) => (
                  <tr
                    key={`${deployment.name}:${deployment.url}:${index}`}
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5">
                      <span
                        className={`text-[12px] font-medium ${getStateColor(
                          deployment.state
                        )}`}
                      >
      ...
      <section>
        <h2 className="text-[13px] font-semibold text-text-primary mb-2">
          Known Startup Endpoints
        </h2>

        {startupEndpoints.length === 0 ? (
          <div className="border border-border-subtle rounded-md px-4 py-5">
            <p className="text-[13px] text-text-secondary">No linked deployment URLs.</p>
            <p className="text-[12px] text-text-tertiary mt-1">
              Startup URLs appear here when a project is linked to Vercel and has a real <code className="font-mono text-[12px] text-text-secondary bg-bg px-1 py-0.5 rounded">.vercel/project.json</code>.
            </p>
          </div>
        ) : (
          <div className="border border-border-subtle rounded-md divide-y divide-border-subtle">
            {startupEndpoints.map((startup) => (
              <div
                key={startup.path}
              ...
              >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="ml-auto text-[12px] text-accent hover:underline font-mono"
                >
                  {startup.deployUrl}
                </a>
              </div>
