...
export default function AgentsPage() {
  ...
  return (
    <div className="px-6 py-5 max-w-5xl">
      ...
      {loading ? (
        <p className="text-[13px] text-text-tertiary py-4">Loading from tmux...</p>
      ) : error ? (
        ...
        <div className="border border-border-subtle rounded-md px-4 py-5">
          ...
          <p className="text-[12px] text-text-tertiary mt-1">
            Start agents with <code className="font-mono text-[12px] text-text-secondary bg-bg px-1 py-0.5 rounded">harness init</code>
          </p>
        </div>
      ) : (
        <>
            ...
            {STATUS_FILTERS.map((f) => {
              ...
              return (
                <button key={f.value} onClick={() => setFilter(f.value)} role="tab" aria-selected={filter === f.value}
                  ...
                  {f.label} <span className="text-[11px] text-text-tertiary ml-1">({count})</span>
                </button>
              );
            })}
          </div>
                ...
                {filtered.map((agent) => (
                  <tr key={agent.paneId} className="border-b border-border-subtle last:border-0 hover:bg-surface-hover transition-colors">
                    ...
                    <td className="px-4 py-2.5 max-w-sm">
                      {agent.lastOutput ? (
                        <pre className="text-[11px] text-text-secondary font-mono whitespace-pre-wrap leading-relaxed line-clamp-2">{agent.lastOutput}</pre>
                      ) : (
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <span className={`text-[12px] font-medium capitalize ${agent.status === "running" ? "text-positive" : "text-text-tertiary"}`}>
                        {agent.status}
                      </span>
                    </td>
                  </tr>
