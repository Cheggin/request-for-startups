...
function NavLink({ item, isActive }: { item: NavItem; isActive: boolean }) {
  return (
    <Link
      ...
      className={`flex items-center gap-2.5 px-2.5 py-[6px] rounded-md text-sm transition-colors ${
        isActive
...
export function Sidebar() {
  ...
  return (
    <aside className="w-44 shrink-0 border-r border-border bg-surface flex flex-col h-full">
      <div className="px-4 py-3.5">
        <span className="text-md heading-page text-text-primary tracking-tight">
          Harness
        </span>
      </div>

      <nav className="flex-1 px-2 py-1 space-y-4" aria-label="Main navigation">
        {NAV_GROUPS.map((group) => (
          <div key={group.label}>
            <p className="px-2.5 pb-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-text-tertiary">
              {group.label}
            </p>
