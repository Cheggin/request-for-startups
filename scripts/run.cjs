...
function resolveHookTimeoutMs(targetPath, extraArgs) {
  ...
  try {
    ...
    // Adapted for startup-harness: hooks live in hooks/ rather than scripts/.
    // Accept either dir name so this file stays a clean copy of OMC's run.cjs
    // minus this one hook-layout adjustment.
    const scriptPattern = new RegExp(`[/\\\\](?:scripts|hooks)[/\\\\]${escapeRegex(scriptName)}(?:\\s|$)`);
