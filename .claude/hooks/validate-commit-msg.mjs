#!/usr/bin/env node
/**
 * Conventional Commits validator — PreToolUse hook on Bash `git commit`.
 * Blocks commits whose subject doesn't match the schema in
 * .harness/commit-schema.md. Exit 0 = allow, Exit 2 = block with message.
 *
 * Ported from packages/hooks/src/validate-commit-msg.ts to drop the bun
 * runtime dependency; behaviour is identical.
 */
