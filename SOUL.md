...
- Built so far: 121 shipped skills (37 absorbed from oh-my-claudecode by direct merge + 84 harness-specific) and 32 agents exposed through the startup-harness v1.0.0 Claude Code plugin. All 11 Claude Code hook event types are wired (UserPromptSubmit, SessionStart, PreToolUse, PermissionRequest, PostToolUse, PostToolUseFailure, SubagentStart, SubagentStop, PreCompact, Stop, SessionEnd) with the full set of enforcement, persistence, and rules-injection hooks. 4 deterministic skill-chain flows enforced by hooks/skill-chain-enforcer.mjs (website-end-to-end, startup-end-to-end, deploy-loop, debug-loop). The plugin bundles OMC's compiled TypeScript runtime (dist/, 476 js files), the bridge daemons (team-bridge, team-mcp, runtime-cli, mcp-server, cli), and registers its own MCP server as 't' — tools like state_*, notepad_*, wiki_*, lsp_*, ast_grep_*, python_repl all resolve to this plugin, not to a separately-installed OMC. Every agent has a scope config in .harness/agents/*.json gating which paths it may write. 55 node:test assertions verify the hook ports against the original vitest fixtures; GitHub Actions runs that suite + MCP-handshake smoke on every push and PR.
